...
original_images_dir = "data/train2014"  # used only for file names
...
for img in sorted_images[:num_images]:
    ...
    # Use the first caption if available.
...
###############################################
# 3. Process and Save Generated Images Embeddings for each CFG scale
###############################################
# List of CFG scales for generated images.
cfg_scales = [1, 3, 7, 10, 20]
base_folder_prefix = "data/generated_sd1_4_"

for cfg in cfg_scales:
    folder_name = f"{base_folder_prefix}{cfg}"
    if not os.path.exists(folder_name):
        print(f"Folder {folder_name} does not exist. Skipping CFG scale {cfg}.")
        continue
    ...
    for file_name in sorted(os.listdir(folder_name)):
        ...
        print(f"[Generated CFG {cfg}] Processing {file_name}: {caption}")
    ...
    save_filename = f"CLIP_{model_name.replace('/', '_')}_embeddings_cfg_{cfg}.npz"
    np.savez(save_filename,
             image_embeddings=image_embeddings,
             text_embeddings=text_embeddings,
             captions=captions_array,
             file_names=file_names_array)
    print(f"Saved generated embeddings for CFG {cfg} to {save_filename}\n")

...
def create_dataset(data_dir, annotation_file, processor, max_samples=None, gen_number=0):
    """Create a dataset for training the VLM.
    
    For generation 0, use original COCO captions.
    For generation 1+, use the same images but with captions generated by the previous generation's model.
    """
    # Load COCO captions
    coco_captions = load_coco_captions(annotation_file)
    
    # Always use images from gen_0 directory
    gen0_dir = "data/vlm_training/gen_0"
    image_paths = glob.glob(os.path.join(gen0_dir, "*.jpg"))
    if not image_paths:
        image_paths = glob.glob(os.path.join(gen0_dir, "*.png"))
    
    logger.info(f"Found {len(image_paths)} images in {gen0_dir}")
    ...
    if gen_number == 0:
        # Use original COCO captions for gen_0
        for image_path in image_paths:
            # Extract the COCO image ID from the filename
            # Filename format is typically COCO_train2014_000000XXXXXX.jpg where XXXXXX is the ID
            filename = os.path.basename(image_path)
            
            # Try different extraction methods to handle various filename formats
            if "COCO_train2014_" in filename:
                # Standard COCO format
                image_id = filename.split("COCO_train2014_")[1].split(".")[0]
            ...
            logger.debug(f"Extracted image ID: {image_id} from {filename}")
            
            if image_id in coco_captions:
                # Use the first caption for simplicity
                caption = coco_captions[image_id][0]
                dataset.append({
                    "image_path": image_path,
                    "caption": caption
                })
            else:
                logger.warning(f"No caption found for image ID: {image_id} from {filename}")
    else:
        # Use generated captions from previous generation
        prev_gen = gen_number - 1
        captions_file = f"data/vlm_captions/gen_{prev_gen}/all_captions.json"
        ...
        for item in generated_captions:
            ...
            # Skip items with error captions
            if not caption.startswith("Error:"):
                image_to_caption[image_path] = caption
        
        # Create dataset with the same images but with generated captions
        for image_path in image_paths:
            # Normalize path to handle different path separators
            norm_path = os.path.normpath(image_path)
            
            # Try different path formats to match with generated captions
    ...
    logger.info(f"Created dataset with {len(dataset)} image-caption pairs for gen_{gen_number}")
...
def train_vlm(gen_number, input_model_path=None, epochs=10, batch_size=4, learning_rate=5e-5, skip_training=False):
    """Fine-tune a Vision-Language Model for captioning."""
    # Set up paths
    data_dir = "data/vlm_training/gen_0"  # Always use gen_0 images
    output_dir = f"models/vlm/gen_{gen_number}"
    ...
    dataset = create_dataset(data_dir, annotation_file, processor, gen_number=gen_number)
    
    # Create training dataset
    train_dataset = VLMCaptioningDataset(dataset, processor)
    
    # Use all images for training instead of just 1
    logger.info(f"Using all {len(train_dataset)} images for training in gen_{gen_number}")
    ...
    if skip_training:
        ...
        try:
            ...
            logger.info(f"Starting training for generation {gen_number} with {epochs} epochs")
            try:
                trainer.train()
...
def generate_captions(model_path, gen_number, image_prompt_pairs, output_dir=None):
    ...
    if output_dir is None:
        output_dir = f"data/vlm_captions/gen_{gen_number}"
    ...
    # Save all results
    with open(os.path.join(output_dir, "all_captions.json"), "w") as f:
...
def run_generation_loop(target_generation=3, epochs=10, batch_size=4):
    """Run the VLM fine-tuning loop for multiple generations.
    
    Generation 0 is the pretrained model (no fine-tuning)
    Generation 1+ are fine-tuned versions
    
    For each generation:
    1. Always use the same 100 images from gen_0
    2. For gen_0, use original COCO captions
    3. For gen_1+, use captions generated by the previous generation's model
    """
    ...
    if current_gen == -1:
        ...
        if os.path.exists(os.path.join(model_path, "config.json")):
            ...
            current_captions_file = f"data/vlm_captions/gen_{current_gen}/all_captions.json"
            if not os.path.exists(current_captions_file):
                logger.info(f"Generating captions for generation {current_gen}")
                # Get all images from gen_0 (always use the same images)
                gen0_dir = "data/vlm_training/gen_0"
                image_paths = glob.glob(os.path.join(gen0_dir, "*.jpg"))
                if not image_paths:
                    image_paths = glob.glob(os.path.join(gen0_dir, "*.png"))
                ...
                # Generate captions for all images
                captions_dir = f"data/vlm_captions/gen_{current_gen}"
...
def main():
    ...
    logger.info(f"Final model saved at: models/vlm/gen_{final_gen}")
